...
for run in range(runs):
    exps = [0.99, 0.999, 0.9999]

    for exp in exps:
        OUR_SMALL_AGENT = True
        pokemon_caught = 1

        file_path_ppo = f"models/log_ppo_dqn_official_test_run_{run}_exp_{exp}.txt"
        ...
        small_agent = _our_model.DQNAgent(env, f"exp{exp}_run_{run}", 1, exp, 0.7)
