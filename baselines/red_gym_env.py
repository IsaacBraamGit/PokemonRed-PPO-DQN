...
class RedGymEnv(Env):
    ...
    def get_levels_sum(self):
        ...
        # todo: change to weighted onj pokemon usefulness
        poke_levels = [poke_levels[0]]+[0]+[0]+[0]+[0]+[0]


        level_addresses = [0xD16B, 0xD193, 0xD1BB, 0xD1E3, 0xD20B, 0xD233]

        # Calculate the addresses for the Pokedex numbers, which are 0x08 bytes before the level addresses
        pokedex_addresses = [addr for addr in level_addresses]

        # Read the Pokedex numbers from the calculated addresses
        pokedex_numbers = [self.read_m(addr) for addr in pokedex_addresses]

        #print(pokedex_numbers)
